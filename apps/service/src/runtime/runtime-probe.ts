...
const pickLessonState = (html: string, pageTitle: string | null): LessonState => {
...
export const probeRuntimeStatus = (snapshot: PageSnapshot): RuntimeStatus => {
  ...
  const questions = extractQuestionsFromHtml(html, extractCourseTitle(html, snapshot.pageTitle));
  ...
  return {
    ...
    lessonState: pickLessonState(html, snapshot.pageTitle),
